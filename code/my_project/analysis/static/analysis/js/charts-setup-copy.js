...
function applyTheme(chart, data, type, theme) {
  ...
  // Calculate totals for stacking percentages
// Calculate totals for stacking percentages per language category
const totalData = data.languages.map((_, idx) =>
  data.genres.reduce((sum, _, sid) => sum + data.data[sid][idx], 0)
);
  // Define color set
  const colorSet = [
  ...
  options = {
    ...
    series: data.genres.map((name, sid) => ({
      name,
      type: 'bar',
      stack: 'total',
      barWidth: '60%',
      ...
      data: data.data.map((row) => (totalData[sid] > 0 ? row[sid] / totalData[sid] : 0)) // Normalize values
    }))
  };
}
...
// Map options
  ...
  options = {
    geo: {
      ...
      itemStyle: {
        areaColor: '#e7e8ea', // Default area color
        borderColor: '#999', // Country border color
      },
      emphasis: {
        itemStyle: {
          areaColor: '#fdb76a', // Highlight color on hover
        },
      },
    },
    tooltip: {
      trigger: 'item',
      formatter: function (params) {
        // Display country name and value in tooltip
        const countryData = data.find((d) => d.name === params.name);
        if (countryData) {
          return `${params.name}<br>Total Movies: ${countryData.value}`;
        }
    ...
    series: [
      {
        type: 'map',
        map: 'world', // Use the registered GeoJSON map
        roam: true,
        data: data.map((country) => ({
          name: country.name,
          value: country.value, // Country value for coloring
        })),
      },
    ],
  };
